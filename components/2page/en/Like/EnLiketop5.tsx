...
const EnLiketop5: FC<Props> = (props: Props) => {
  ...
  function func_post(e: number) {
    ...
    if (memberid === 0) {
      ...
      axios
        .post(
          process.env.REACT_APP_DB_HOST + `/api/en/myplace/add/${memberid}/${props.top5placeid[e]}`,
          { memberid, ethplaceid },
  ...
  function func_delete(e: number) {
    axios
      .delete(process.env.REACT_APP_DB_HOST + `/api/en/myplace/deletebyplace/${memberid}/${props.top5placeid[e]}`)
      .then(() => {})
      .catch((error) => {});
  }
